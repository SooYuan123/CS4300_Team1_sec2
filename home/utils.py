...
# Radiant Drift API Configuration
...
def fetch_rise_set_times(body, latitude, longitude, from_date=None, to_date=None):
    ...
    # Radiant Drift rise-set endpoint
    ...
    try:
        response = requests.get(url, headers=get_radiant_drift_auth_header(), params=params, timeout=10)
        if response.status_code == 404:
            return []
        response.raise_for_status()
        data = response.json()
        
        # Transform Radiant Drift response to match expected format
        events = []
        if 'response' in data:
...
def fetch_body_position(body, date_time, latitude, longitude):
    ...
    Args:
        body: 'sun', 'moon', or 'galactic-center'
...
def fetch_astronomical_events(body, latitude, longitude, elevation=0, from_date=None, to_date=None):
    """
    Fetch astronomical events. Uses Radiant Drift for sun/moon, returns empty for planets.
    This maintains compatibility with existing code.
    """
    body_lower = body.lower()
    
    # Radiant Drift supports sun and moon
    if body_lower in ['sun', 'moon']:
        return fetch_rise_set_times(body_lower, latitude, longitude, from_date, to_date)
    
    # For planets, we can't get events from Radiant Drift
...
def fetch_moon_phase(date_time, latitude, longitude):
    ...
    Args:
        date_time: ISO datetime string or datetime object
        latitude: Observer latitude
        longitude: Observer longitude
...
def fetch_solar_eclipse_data(from_date=None, to_date=None):
    ...
    try:
        response = requests.get(url, headers=get_radiant_drift_auth_header(), timeout=10)
        response.raise_for_status()
